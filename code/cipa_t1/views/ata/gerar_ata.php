...
<body>
    ...
    <div class="header">
        ...
        <div class="header-actions">
            ...
            <button onclick="enviarAtaParaFuncionarios()" class="btn-link" style="border: none; cursor: pointer; background-color: #28a745; color: white; margin-top: 0px; margin-left: 10px;">Enviar ATA para Funcionários</button>
                ...
                <tbody>
                    <?php foreach ($_SESSION['ata_candidatos'] as $candidato): ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($candidato['cargo_candidato'] === 'Indicado'): ?>
                                    <span style="font-style: italic; color: #666;">Candidato Indicado</span>
                                <?php else: ?>
                                    <strong><?php echo htmlspecialchars($candidato['quantidade_voto_candidato']); ?></strong>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
    ...
    <script>
        function enviarAtaParaFuncionarios() {
            // Mostrar alert de carregamento
            alert('Aguarde alguns segundos, a ata está sendo enviada...');
            
            // Criar formulário dinamicamente
            var form = document.createElement('form');
            form.method = 'POST';
            form.action = '/code/cipa_t1/ata/enviar';
            
            // Adicionar ID da eleição
            var input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'id_eleicao';
            input.value = '<?php echo htmlspecialchars($_SESSION['ata_eleicao']['id_eleicao']); ?>';

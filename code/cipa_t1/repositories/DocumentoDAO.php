...
class DocumentoDAO extends Conexao {
    ...
    public function podeSerDeletado(int $idDocumento) {
        try {
            // Verificar se pode ser deletado (não está vinculado a nenhuma eleição ATIVA)
            $sql = "SELECT COUNT(*) FROM eleicao 
                    WHERE documento_fk = :id_documento 
                    AND status_eleicao = 'ABERTA'";
            $stmt = $this->db->prepare($sql);
            ...
            $temEleicaoAberta = $stmt->fetchColumn() > 0;
            
            if ($temEleicaoAberta) {
                error_log("Documento $idDocumento NÃO pode ser deletado - está vinculado a eleição ABERTA");
                return false;
            }
            
            error_log("Documento $idDocumento PODE ser deletado - não está vinculado a eleição ABERTA");
            return true;
        } catch (PDOException $e) {
    ...
    public function deletar(int $id) {
        try {
            // PRIMEIRO: Verificar se pode ser deletado (não está vinculado a eleição ATIVA)
            if (!$this->podeSerDeletado($id)) {
                error_log("Tentativa de deletar documento $id vinculado a eleição aberta - BLOQUEADO");
                return false;
            }
            ...
            // Buscar eleições vinculadas ao documento (apenas eleições finalizadas)

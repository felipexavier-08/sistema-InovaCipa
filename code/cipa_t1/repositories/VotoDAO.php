...
class VotoDAO extends Conexao {
        ...
        public function funcionarioJaVotou(int $idFuncionario, int $idEleicao) {
            try {
                // Verificar diretamente na tabela de votos (mais confiável)
                $sql = "SELECT COUNT(*) 
                        FROM voto v
                        INNER JOIN eleicao e ON v.data_hora_voto BETWEEN e.data_inicio_eleicao AND e.data_fim_eleicao
                        WHERE v.usuario_fk = :usuario_fk 
                        AND e.id_eleicao = :id_eleicao";
                
                $stmt = $this->db->prepare($sql);
                $stmt->bindValue(":usuario_fk", $idFuncionario, PDO::PARAM_INT);
                $stmt->bindValue(":id_eleicao", $idEleicao, PDO::PARAM_INT);
                $stmt->execute();
        ...
        public function contarEleitoresQueVotaram(int $idEleicao) {
            try {
                ...
                error_log("DEBUG: Contando eleitores que votaram - Eleição: $idEleicao, Total: $total");
